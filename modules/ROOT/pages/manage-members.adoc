...
NOTE: Registered members require a license. No license is required when you create a guest member. For full information about licenses refer to the xref:licensing-overview.adoc[Licensing Overview].
...
approve the member registration, navigate to the API community *ACM Administrator* interface.
. Select *Manage Members*. This section provides the status of all consumer developer registration requests.
. Select *Approve* to activate the member and trigger a welcome email.
...
and *My Applications*.
. (Optional) To deactivate a member, disable the *Active* setting for that member.
