...
== Chanages to APIs Are Not Displaying
...
To fix this error, follow the installation prerequisite steps in the section xref:installation-prerequisites.adoc#set-up-a-domain-with-enabled-communities[Set Up a Domain with Enabled Communities].
...
== Troubleshooting Branded Emails

 The following information includes symptoms and troubleshooting steps for branded emails.

=== Data Capture Process Fails

When the change data capture is enabled, Salesforce starts polling the ACM Community Bridge for contract updates. If the data capture process job fails, Salesforce stops the polling. In some cases, the solution is for the administrator to resume the data capture process, but first check the status of the data capture process.

*Check the status of the data capture process*

A symptom that the data capture process failed is the status of the job execution can be *Waiting*. To check the status of the data capture process, execute the following SOQL code in the developer console:
