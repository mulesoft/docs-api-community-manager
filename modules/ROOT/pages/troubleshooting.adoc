...
. Click the name of the Anypoint Platform authentication provider and ensure that it has these settings from the correct connected app in Anypoint Platform:
  ...
  . Ensure that the authorize endpoint URL, token endpoint URL, and default scopes are configured as described in xref:configure-acm.adoc#create-your-authentication-provider[Create Your Authentication Provider].
...
. Ensure that the Anypoint Platform credentials are configured as described in xref:configure-acm.adoc#authorize-and-authenticate-your-external-data-source[Authorize and Authenticate Your External Data Source].
