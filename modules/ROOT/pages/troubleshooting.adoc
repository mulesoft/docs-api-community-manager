...
. Then open your community's Remote Sites Settings.
. Ensure that the list also includes the endpoint site domain.

The console works in the user's browser and can call only trusted endpoint sites.

Since wildcards are not allowed you need to specify all sub-domains used for real API endpoints, not just the main domain.
