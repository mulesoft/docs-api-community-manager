...
If installation of Anypoint API Community Manager shows a `Missing feaure` error, ensure communities are enabled in your API Community Manager Salesforce organization.
...
To fix these errors, follow the installation prerequisite steps in the section xref:installation-prerequisites.adoc#set-up-a-domain-with-enabled-communities[set up a domain with enabled communities].
