...
. Open your community's Remote Sites Settings.
. Ensure that the list includes the endpoint site domain.

The console works in the user's browser and can call only trusted endpoint sites.

Wildcards are not allowed, so you must specify all sub-domains used for real API endpoints, not just the main domain.
