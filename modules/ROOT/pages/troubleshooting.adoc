...
. Click the name of the Anypoint Platform authentication provider and ensure that it has these settings from the correct connected app in Anypoint Platform:
  . Ensure the consumer key and consumer secret match the connected app in Anypoint Platform.
  . Ensure the authorize endpoint URL, token endpoint URL, and default scopes are configured as described in  xref:configure-acm.adoc#create-your-authentication-provider[Create Your Authentication Provider].
  ...
  . Ensure your connected app in Anypoint Platform has the same domain as your Salesforce organization in its `redirect_uri` setting, a client ID and client secret that match those in Salesforce, and correct scopes.
...
. Ensure the API version is managed in API Manager.
...
In the community builder, check the audience targeting settings of the xref:acm-lightning-components.adoc#api-access-requester[API Access Requester] Lightning component. To make the API Access Requester appear to all members, ensure it is not assigned to any audience. To make it appear only to a specific audience, ensure it is assigned to the correct audience, and verify it by viewing the page while logged in to an account that is a member of this audience.
...
. If the image was uploaded to Exchange by using the markdown editor, make the Exchange API version asset public so the image can be rendered in your community.

. If the image is stored on a web location outside Exchange, such as S3 or Dropbox, then:

. Open Salesforce setup.
. Open your community's content security policy (CSP) trusted sites list.
. Ensure the list includes the image site domain.
. Because some file-sharing services resolve URLs through multiple redirects, ensure the list includes the domain of the final URL.
...
. Ensure the Anypoint Platform credentials are configured as described in xref:configure-acm.adoc#authorize-and-authenticate-your-external-data-source[Authorize and Authenticate Your External Data Source].
...
. Ensure the sharing rule for the object `CommunityApi` matches your community name exactly, including spacing and lettercasing.
