...
If saving the external data source configuration shows one of the errors "Not authorized" or "Problem Logging In - No_Oauth_State: State was not sent back":
...
. Click the name of the Anypoint Platform authentication provider and ensure that it has these settings from the correct connected app in Anypoint Platform.
  ...
  . Ensure that the authorize endpoint URL, token endpoint URL, and default scopes are configured as described in the section xref:configure-acm.adoc#create-your-authentication-provider[Create Your Authentication Provider] on the page Configure ACM.
  ...
  . Ensure that your connected app in Anypoint Platform has the correct domain of your Salesforce organization in the `redirect_uri` setting, client ID and client secret that match those in Salesforce, and correct scopes.
...
. Ensure that the API version is managed in API Manager. If it is not, click *Import from Exchange*.
...
In the community builder, check the audience targeting settings of the xref:acm-lightning-components.adoc#api-access-requester[API Access Requester] Lightning component. To make the *API Access Requester* appear to all members, ensure that it is not assigned to any audience. To make it appear only to a specific audience, ensure that it is assigned to the correct audience, and verify it by viewing the page while logged in to a member account that is a member of this audience.
...
. Some file sharing services resolve URLs through multiple redirects. Ensure that the list includes the domain of the final URL.
...
. Ensure that the Anypoint Platform credentials are configured as described in the section xref:configure-acm.adoc#authorize-and-authenticate-your-external-data-source[Authorize and Authenticate Your External Data Source] on the page Configure ACM.
...
. Ensure that the sharing rule for the object `CommunityApi` matches your community name, including spaces and which letters are upper case.
