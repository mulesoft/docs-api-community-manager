...
.. Ensure that the authorize endpoint URL, token endpoint URL, and default scopes are configured as described in xref:data-bridge.adoc#create-auth-provider[Create an Authentication Provider in Your Salesforce Organization].
...
. Ensure that the Anypoint Platform credentials are configured as described in xref:data-bridge.adoc#authenticate-data-bridge[Authenticate the Data Bridge].
...
If the action of validating and synchronizing the external data source is performed, this will cause the external objects to be duplicated, generating new objects that are not going to be used, in case this action is performed, the new objects must be deleted.