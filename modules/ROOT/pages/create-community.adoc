...
After you create a community, you must create different user profiles for your community _members_ (signed in) and _guests_, must be created to control access to and visibility of APIs and content.
...
API Community Manager uses the following permission sets to define the visibility and access to external objects and Apex classes. 
...
=== Configure Member User Permission Set 
...
If you want to create an new user, you must use the API Community Diagnostics or manually set the member user profile to the *ACM Member User* permission set.
...
[[guest-user-perms]]
=== Configure Guest User Permission Set

The permission for the guest user must be assigned to the *ACM Guest User* permission set so your guests can access APIs, create client applications, and manage their details. 
