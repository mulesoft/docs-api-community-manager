...
Use these steps to create, configure, and publish your community with branding, forums and moderation rules, Salesforce CMS, Salesforce CRM Content, and Chatter tracking for APIs.
...
. If you want help designing your community, choose any of the *API Community Manager Templates* or any of the other available templates.
. If you don't need help designing your community, choose *Build your own*. 
. Click *Get Started*.
...
+
. When the configuration process finishes and you see your new community workspace, click *Install AppExchange Package* in the top banner and follow the installation wizard instructions.
. Click *Install AppExchange Package* on the top banner to install the pre-defined metrics and dashboards, and follow the installation wizard instructions.
+
Be sure to install this package in your production organization when prompted by the wizard.
+
You may need to re-enter your login credentials to complete the installation of this package.
...
Chatter tracking enables your community to create discussions on specific objects like APIs. To enable Chatter tracking:
...
Users that are signed in are *members*, and other users are *guests*.
...
. In *Setup*, navigate to the guest user profile link under the *Guest user profile* section, named *[Community Name] Profile*.
. Click *Edit*.
. Edit the object permissions to match the table.
. Click *Save*.
. Click the link for *Enabled Apex Class Access*.
. Click *Edit*.
. Select all classes on the list and click *Add* to make them accessible.
. Click *Save*.

=== Member User Profile Permissions
...
== Set Up an Admin Approval Process for New User Creation (Optional)
...
Re-authenticate the data bridge as described in xref:data-bridge.adoc#authenticate-data-bridge[Authenticate the Data Bridge].
