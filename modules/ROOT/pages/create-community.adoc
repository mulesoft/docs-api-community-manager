...
Configuring profiles and permissions includes configuring user profiles for guests and members, sharing settings, member registration, and user impersonation.

=== Required Profiles and Permissions
...
These are the permissions for the guest user:
...
. Click *Edit*.
. Edit the permissions to match the table.
. Click *Save*.
...
Re-authenticate the data bridge as described in <<authenticate-data-bridge>>.
