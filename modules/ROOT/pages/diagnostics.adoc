...
The *Status* tab shows the status of your Salesforce *Organization* and your *ACM Communities*.
...
Each category lists its configuration items. Items that are not yet configured correctly have an error and a *Fix* button, and their category has a *Fix All* button. Correctly configured items and categories have no button and a green circle with a white check mark.
...
The *Member User* item *Account assigned* has an *Assign* button and not a *Fix* button. If an attempt to assign the Salesforce account automatically does not work, assign manually:
...
. Select your community's member user as the *Profile*.
. Enter your community's Salesforce account name as the *Account*.
. Click *Save*.
