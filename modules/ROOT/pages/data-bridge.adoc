...
. Set *JWT Signing Certificate* to the key that you previously created in the  Create Key Pair section.
...
. Click Save.
. Click on the Name of the External Data Source to enter to the detail.
...
. In *Exchange Viewers*, set a new External group called `Community User` and save it (to perform this step it is necessary to have exchange administrator permissions).
