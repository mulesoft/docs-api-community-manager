...
. In the Salesforce Organization go to *Setup*, search for *Identity Provider* using the *Quick Find* box and click *Identity Provider*.
. Click *Enable Identity Provider*.
. Using the *Quick Find* box search for *App Manager*, search the App named *Anypoint* and in the last colum of the table, hit the arrow and click *Manage*.
...
. Set the *Public Key* to the public key extracted from the `<ds:X509Certificate>` tag in the metadata XML you downloaded. The certificated must have the correct format. i.e. 
----  
-----BEGIN CERTIFICATE-----   
PUBLIC KEY
-----END CERTIFICATE-----  
...
. Set *URL* to `callout:acm_pkg__anypoint_mulesoft/odatabridge/odata.svc/`.
. Go to the *Authentication* section.
. Set *Identity* to *Anonymous*.
. Set *Authentication Protocol* to *No Authentication Protocol*.
. In the section *Custom HTTP Headers*, for the *Header Field Name* *X-FirstName*, ensure *Active* is disabled.
. Click *Save*.
