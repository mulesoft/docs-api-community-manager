...
This issue can occur when the guided setup fails to create the data bridge between Anypoint and API Community Manager when an organization has multiple identity providers. During setup, an API request is sent to obtain the identity provider ID and to check if it matches the ID that is entered. When there are more than one identity providers, only the first identity provider ID from the response is read, which might not match.
