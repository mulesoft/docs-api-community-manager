...
To create a connected app with an API call, follow this example using `cURL`. Remember to retrieve an authorization token from the platform:
    `curl -X POST \
...
=== Configure Salesforce as a SAML Identity Provider [[configure-idp]]
...
=== Configure External Identity in your Anypoint Organization[[configure-anypoint-account-with-external-identity-with-saml]]
...
After you finish all the configuration, make sure you re-authenticate the Exernal Data Source as described in <<authenticate-the-external-data-source>>.
