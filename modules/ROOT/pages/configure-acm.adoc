...
The *ACM Control Panel* is the interface your community administrators and operators will use to operate your communities. It is available from the *App Launcher* (the first icon displayed in the main navigation bar(. To ensure it is available for admin users:
...
. Select *Navigation Items* on the left panel, highlight *ACM Administrator* item under *Available Items* box, and use the arrow buttons to move it to the *Selected Items* box. Select *Save*.
. Click *Back* on the top right corner of the screen.
. Log out and log back in to your Salesforce organization.
. Open the *App Launcher* and select *API Community Manager*.
. Verify that the *ACM Control Panel* is displayed correctly. Observe that you'll see a message indicating you have not created a community. If you are able to see and open *ACM Administrator* in the *Application Launcher* and you see this message, it means the actions in this step were done correctly.

//ADD SCREENSHOT TO SHOW HOW THE *ACM Control Panel* SHOULD LOOK LIKE IF ITS ENABLED CORRECTLY


== Update Your User Role and Permissions [[verify-organization-user-role-setting]]

. In *Setup*, search for *Users* using the *Quick Find* box and select *Users*. 
. Locate your username and select *Edit*.
. Set the *Role* for your account. This can be any role from the list, but cannot be none. If you don't see any roles in the dropdown list, refer to https://help.salesforce.com/articleView?language=en_US&type=1&mode=1&id=000322715[this knowledge article, role=external, window=_blank] that describes how to create a role in your organziation
...
. Choose any of *API Community Manager Templates* if you want to get started from an example, any of the other available templates, or *Build your own* if you want to design your community from scratch with full control of the structure and look&feel. 
...
. Once the configuration process finishes, you'll be in your new community workspace. Select *Install AppExchange Package* on the top banner to install the pre-defined metrics and dashboards, and follow the instructions of the installation wizard. Make sure you install this package in your production org when prompted by the wizard. Also, observe you may need to re-enter your login credentials to complete the installation of this package.
. Navigate to *ACM Administrator* in the *Application Launcher*, select *Community Administration*, go to the *Settings* section in the left panel, and select *Activate Community*.
...
. Navigate to *ACM Administrator* in the *Application Launcher*, select *Community Administration*, and select *Preferences* in the left panel.
. Select *Show all settings in Workspaces*.
. Select *Save* so that this setting is reflected. 
...
. Create as many topics as you need for your community forums and make sure you select *Save*.

=== Define Moderation Rules for Forums [[define-moderation-rules-for-forums]]

Discussion Forums are by default not moderated, which means any member can create posts with any content and they become visible for other members right away. If you want to define a basic "review/approve-all" moderation model, follow the steps below:
...
. In the *Rule Conditions* section, seletc *Post* and *Comment*, and select *Review* in *Moderation Action*. 
. In the *Criteria* section, move all items in *Member Criteria* from *Available Criteria* box to *Selected Criteria* box.
. Select *Save*.
...
You can define much more advanced moderation and review rulewsa. You can find more information in the Salesforce Community Cloud documentation about https://help.salesforce.com/articleView?id=networks_moderator_manage_rules.htm&type=5[forum moderation rules, role=external, window=_blank].
...
. Select *User Profiles* on the left panel, highlight *System Administrator* item under *Available Profiles* box, and use the arrow buttons to move it to the *Selected Profiles* box. 
. Select *Save*.
. Click *Back* on the top right corner of the screen.
...
=== Publish your Community [[publish-a-community]]

. In the *ACM Control Panel*, select *Community Builder*.
. Select *Publish* on the top right corner, and select *Publish* in the *Publish YUour Community* dialog.
. In a few minutes your community will be live, and you will receive an email notification including your community's public URL. You'll also find your community live URL in the *ACM Control Panel*, by selecting *Open Community*. 
...
=== Configure Sharing Settings [[create-sharing-settings]]
...
. Select Save.
. Switch to Salesforce Classic by selecting *Switch to Salesforce Classic* under the user profile menu on the top right of the screen.
. Select the plus (+) button at the end of the navigation menu bar
. In *View* dropdown, select *Content* and then select *Libraries*.
...
. Switch back to the lightning Experience by selecting *Switch to Lightnign Experience*.
...
=== Configure Guest User Access [[open-community-for-guest-access]]

. In the *ACM Control Panel*, open *Community Builder*, select *Settings* -> *General* 
. Select the *Public can access the community* check box.
. Navigate to the guest user profile link under *Guest user profile* section, named *[Community Name] Profile*.
. Select *Edit*.
...
=== Enable Guest User Registration as Members [[enable-external-user-registration]]

==== Add Self Register Component to Registration Page [[add-self-register-custom-component-to-register-page]]

The self-register component allows guest users to self-register in your community to become members. If you're not using any of the ACM out-of-the-box templates, follow these steps to use ACM-specific self-registration component.
...
==== Assign a Profile for your Community Members [[assign-the-profile-to-new-community-members]]
...
IMPORTANT: Do not *Remove* the *System Administrator* profile from the *Selected Profiles* section. If you do you will lose all acceess to your community.

==== Create an Account for Your Community [[create-the-account-for-your-community]]
...
==== Configure Your Login and Registration Pages [[configure-your-login-and-registration-page]]
...
===== Create the Approval Process  [[specify-approval-process-parameters]]

. In the *Setup*, search for *Approval Processes* in the *Quick Find* box and then select *Approval Processes*.
...
... Select *Registration Request* in *Approval Assignment Email Template* section, under the *ACM* category in the look-up dialog.
.. Select *Next*.
...
. Select the user who you want to be the approver to open it.
...
To do this through the UI: in Anypoint Platform, navigate to *Access Management* -> *Applications* -> *Create New Application*, and provide the following information:
...
Alternatively, you can perform this step directly through an API call, as in the example below using `cURL`. Remember to retrieve an authorization token from the platform:
    `curl -X POST \
...
. In *Setup*, search for *Auth. Providers* using the *Quick Find* box and select *Auth. Providers*.
. Locate the *Anypoint* item and select *Edit*.
...
Impersonation uses SAML, and it requires  an IdP to be configured in both Anypoint Platform and Salesforce organizations. You can use Salesforce as an IdP.

To set up SAML in your Salesforce org:
...
. Provice the following information in the *Web App Settings* section:
.. Provide the *Start URL*: `https://anypoint.mulesoft.com/accounts/login/<your_anypoint_domain_name>` or the location
where you want users to be sent in the Anypoint Platform. You can find your Anypoint organization domain name under *Access Management* -> *Organization* and selecting the root organization.
.. Select *Enable SAML*.
...
.. Set *Issuer* to *salesforce_org_domainname* (like `https://[YourOrgDomain].my.salesforce.com`.
...
. Set the *Public Key* to the public key extracted from the Metadata XML you downloaded in the `<ds:X509Certificate>` tag.
