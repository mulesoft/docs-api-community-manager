...
NOTE: If your Anypoint Platform organization is hosted in the EU, change `+https://anypoint.mulesoft.com/accounts/api/v2/oauth2/authorize+` to `+https://eu1.anypoint.mulesoft.com/accounts/api/v2/oauth2/authorize+` and `+https://anypoint.mulesoft.com/accounts/api/v2/oauth2/token+` to `+https://eu1.anypoint.mulesoft.com/accounts/api/v2/oauth2/token+` in the Auth Provider configuration section.
...
Re-authenticate the External Data Source as described in <<authenticate-the-external-data-source>>.
