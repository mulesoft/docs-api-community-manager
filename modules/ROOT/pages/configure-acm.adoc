...
. Set *Application Name* to a name for the app.
. Set *Redirect URIs* to the URL from the Authentication Provider configuration.
+
Find this URL in *Salesforce Setup* -> *Auth. Providers* -> *Anypoint* -> *Callback URL*.
+
. Set *User Grant Types*.
+
Select *Authorization Code*, *Refresh Token*, *Full Access*, and *Background Access*.
+
...
. Set *Consumer Key* to the application ID of the connected app created in Anypoint.
+
...
+
. Set *Consumer Secret* to the application secret of the connected app created in Anypoint.
+
