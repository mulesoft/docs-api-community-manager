...
NOTE: If you Anypoing Platform organization is https://docs.mulesoft.com/eu-control-plane/platform-access-eu[hosted in the EU region, role=external, window=_blank] you must replace all occurences of `anypoint.mulesoft.com` domain by `eu1.anypoint.mulesoft.com` in all the steps in this section.
...
. Select *Save*. 
. If you don't have an active session in your Anypoint Platform organization, the *Anypoint Login* page is shown. Log in to the Anypoint Platform to complete the authentication. If you already have an active session the Log in screen will not be presented, but the authentication process will be performed nonetheless. 
. Search for *External Data Source* in the *Quick Find* box and then select *External Data Sources*. 
. Verify the Exchange *External Data Sources* *Authentication Status* is *Authenticated*.

In addition to authenticating the External Data Source, you need to provide credentials of an Anypoint Platform "system" user with read-only access, for components like the API Console which retrieve data directly from the Anypoint Platform APIs:
...
NOTE: If your Anypoint Platform organization is hosted in the EU, change `https://anypoint.mulesoft.com` by `https://eu1.anypoint.mulesoft.com`.



== Enable ACM Control Panel [[give-permissions-to-acm-community-manager]]

ACM Control Panel is the interface your community administrators and operators will use to  operate your communities. It is available from the *App Launcher*: the first icon displayed in the main navigation bar. To ensure it is available for admin users:
...
. Verify ACM Control Panel is displayed correctly


== Verify Organization User Role Setting [[verify-portal-account-owner-role-setting]]

. Navigate to *Setup -> Users* and select your username.
. Set any *Role* or the appropriate *Role* for your case.
. Select *Save*.
...
=== Customize Your Community [[customize-a-community]]

Refer to xref:brand-intro.adoc[Brand and Customize Your API Community].

=== Enable Forums [[enable-forums]]

. In the ACM Control Panel, select *Community Workspace*, then select *Administration* and then select *Preferences*.
...
. In the ACM Control Panel, select *Community Workspace*, then select *Moderation -> Rules*.
...
You can find more information about Forums moderation rules in https://help.salesforce.com/articleView?id=networks_moderator_manage_rules.htm&type=5[Communtiy Cloud documentation]
...
. You will receive an email in a few minutes once your community is live including the public URL of your community. 

NOTE: If you want to use a custom domain name (also known as a *Vanity Domain*) refer to this https://help.salesforce.com/articleView?id=000336819&type=1&mode=1[knowledge article].

=== Enable Salesforce CMS <<enable-cms>>

. Go to *Setup* -> *Salesforce Files* -> *General Settings* and enable *Libraries*.
. To tag CMS content with topics, in the ACM Control Panel, select *Community Workspace*, go to *Content Management* -> *Topics*, and select the topic setting *Enable for content*.
...
=== Configure Member Users [[configure-registered-users]]
...
NOTE: if you ever change the name of your community make sure you update this setting. 

image::access-mapping.png[Access Mapping for CommunityApi]

// More info needed?

=== Open the Community for Guest User Access [[set-permissions-for-unauthenticated-users]]

. In the ACM Control Panel, select *Community Builder*, and navigate to *Settings* -> *General* and select the *Public can access the community* check box.
...
NOTE: If your Anypoint Platform organization is hosted in the EU, change `https://anypoint.mulesoft.com/accounts/oauth2/authorize` to `https://eu1.anypoint.mulesoft.com/accounts/oauth2/authorize` and `https://anypoint.mulesoft.com/accounts/oauth2/token`
to `https://eu1.anypoint.mulesoft.com/accounts/oauth2/token` on the Auth Provider configuration section.



== Additional Configuration for EU Hosted Organizations [[configure-eu-account]]

If your Anypoint Platform organization is hosted in the EU, perform the following additional configurations to enable access to the EU instance of the Anypoint Management Plane. 
