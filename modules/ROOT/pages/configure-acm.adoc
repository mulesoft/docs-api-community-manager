...
ACM Control Panel is the interface your community administrators and operators will use to operate your communities. It is available from the *App Launcher* as the first icon displayed in the main navigation bar. To ensure it is available for admin users:
...
. In the ACM Control Panel, select *Community Workspace* -> *Administration* -> *Preferences*.
...
You can find more information in the Salesforce Communtiy Cloud documentation about https://help.salesforce.com/articleView?id=networks_moderator_manage_rules.htm&type=5[forum moderation rules].
...
NOTE: To use a custom domain name (also known as a *Vanity Domain*) refer to the Salesforce knowledge article https://help.salesforce.com/articleView?id=000336819&type=1&mode=1[Run your Salesforce Community under a custom domain].

=== Enable Salesforce CMS <<enable-cms>>

. Go to *Setup* -> *Salesforce Files* -> *General Settings* and enable *Libraries*.
. To tag CMS content with topics, in the ACM Control Panel, select *Community Workspace* -> *Content Management* -> *Topics*, and select the topic setting *Enable for content*.
