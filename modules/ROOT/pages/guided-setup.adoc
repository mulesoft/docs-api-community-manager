...
+
Otherwise, create the connected app or ask an organization administrator to create it using the following steps:
+
...
+ 
The public key is saved on your local machine and will be required in the next step when creating the connecting app in Anypoint.
+
.. *Create a Connected App in Anypoint*
...
+
You will receive a client ID and a client secret. This configuration uses the certificate and does not use the client secret. Save the client ID so you can use it in the following steps.
...
+
Otherwise, provide the following app details:
+
.. *Client ID*: Copy the Client ID from the connected app that you have created in the previous steps
.. *IDP Issuer*:  In your Anypoint Platform organization, navigate to *Access Management > Identity Providers > SAML 2.0 > Edit* and copy the the IDP Issuer string.
.. *Audience*:  In your Anypoint Platform organization, navigate to *Access Management > Identity Providers > SAML 2.0 > Edit* and copy the the Audience string.
.. Click *Connect*.
