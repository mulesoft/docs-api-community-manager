...
To set up Anypoint API Community Manager with guided setup, complete the xref:installation-prerequisites.adoc[prerequisite steps] and then follow these steps. The prerequisite steps include configuring an identity provider. Using OpenID Connect as your identity provider requires using the xref:manual-setup.adoc[manual setup] and not guided setup.
...
----
+
.. Set *IDP ID* to this identity provider ID.
