...
When the change data capture is enabled, Salesforce starts polling the ACM Community Bridge for contract updates. If the data capture process job fails, Salesforce stops the polling. 
...
See the Optionally Configure Branded Emails topic for complete instructions to set up branded emails.
