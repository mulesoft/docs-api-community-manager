...
. Click *Add* from *User Selectable Organization-Wide Email*.
...
. Click *Add* from *Special Purpose Organization-Wide Email*.
. Set *Display Name* to `EventEmailSender`.
. Set *Email Address* to the email address at your organization that sends the API contract event notification emails.
. Click *Save*.
...
. Navigate to *Setup* > *Home* > *Integrations* > *External Data Sources*.
. Click *Exchange* and ensure *Eligible for External Data Change Capture* is enabled.
...
. Enter *AnypointContracts* in *Available Entities*.
. Move *AnypointContracts* to the *Selected Entities* list, which enables polling on this object.
. Click *Save*.

=== Create Apex Trigger

. Navigate to *Setup* and click *Developer Console*.
. Click *File* > *New* > *Apex Trigger*.
. Set *Name* to `AnypointEmailNotificationsTrigger`.
. Set *sObject* to `acm_pkg\__AnypointContracts__ChangeEvent`.
. Click *Save* to create the new trigger.
...
. Click *Manage Records* for *ACM Community Configuration Metadata*. 
...
. Go to *Setup* and click *Developer Console*.
. Click *Query Editor* to open the editor.
. Enter `SELECT Id, Name FROM Network` and click *Execute*.
. In the list of IDs and communities, select the row with *Name* with to your community's name, double-click the name, and copy it.
. Go to *ACM Community Config Metadata* field and paste the name in *Label* and in *ACM Community Configuration Metadate Name*.
. In to the Query Editor's list of IDs and communities, select the row with *ID* with to your community's ID, double-click the ID, and copy it.
. Go to *ACM Community Configuration Metadata* and paste the name in *NetworkId*.  
...
. In the *ACM Community Configuration Metadata* field *ContractApprovedEmailTemplateId*, paste the ID.
...
. In  *ACM Community Configuration Metadata*  *ContractRevokedEmailTemplateId*, paste the ID.
...
. In  *ACM Community Configuration Metadata* field *OrgWideEmailAddressId*, paste the ID.
. Click *Save*.
...
. Click *Edit* for *Contracts Approved*.
...
The user who enables the external data capture must have permissions to read contracts in Anypoint. Follow these steps to configure the Anypoint permissions.

. Log in to your API community as an administrative user.
. In *Access Management*, click *Users*.
. Click the username link of the impersonated Anypoint user with the email address of the Salesforce administrator.
. Click *Roles*.
. In *Add a role by name*, enter `Organization Administrators` and click *+*.
+
...
. Click *Edit*.
. In the *Per User Subject* formula, add `& ".donotsend" ` after `$User.Email`. 
+
Example: 
+
`"v2|8745gh23-590d-4h80-b339-1637854e289f|" & $User.Username & "|" & $User.FirstName & "|" & $User.LastName & "|" & $User.Email & ".donotsend" & "|[\"Community User\"]"`
