...
is approved or revoked. By default these emails use Anypoint addresses and
...
----
+
. Grant the *Organization Admin* role to the Anypoint user named *Automated Process*.
+
The impersonation configuration you performed created this Anypoint user automatically. It works with the Salesforce user named *Automated Process* to process triggers. The Anypoint user needs the *Organization Admin* role to process the API contract events when API contracts are approved or revoked.
