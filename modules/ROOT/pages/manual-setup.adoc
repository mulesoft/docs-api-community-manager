...
To set up Anypoint API Community Manager manually, complete the xref:installation-prerequisites.adoc[prerequisite steps] and then follow these steps.
...
+
The guided setup uses a slightly different formula in the `IDP_ISSUER` field, but either formula works.
+
...
.. If you are using an identity provider with OpenID Connect:
+
Replace `ORGANIZATION_ID` with the organization ID from your Anypoint account.
+
...
After each user logs in to the community, the user is visible in Anypoint.
