...
If you want to change the default configuration for either part of the cache:
...
. Set the number of seconds that cached data remains in the cache for applications, API versions, documentation content, instances, and queries by completing the associated fields.
. Hover over the question mark (?) next to each field name to see the minimum and maximum number of seconds for each entity.
