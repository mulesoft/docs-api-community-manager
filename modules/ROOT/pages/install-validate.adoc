...
1. Verify that your Anypoint organization has the Anypoint API Community Manager entitlement. Log in to your Anypoint Platform organization and navigate in your browser to `+https://anypoint.mulesoft.com/accounts/api/profile+`. Check that under the *Entitlements* property the `apiCommunityManager` value is `true`.
+
...
+
3. Verify that API versions are shown in the API Community Manager control panel. Open the Application Launcher. Enter *API Community Manager*. Check that the list *APIs in Exchange* is not empty and contains the APIs in your organization.
+
