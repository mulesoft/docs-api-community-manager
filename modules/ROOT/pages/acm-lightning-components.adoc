...
To design your API Community Manager portal, open the Builder and add ACM Lightning Components as described in xref:customize.adoc[Customize the Look and Feel of Your API Community].
...
Endpoints are displayed in alphabetical order, even when the original API specification listed them in a different order.

The console components are the *API Console*, *API Console Documentation*, *API Console Instances*, *API Console Navigation*, and *API Console Request Panel*. In the Builder you can add multiple console components to the same page. If you do this, any change in one console component will also be visible in the other console components on the same page that show the same API. For example, selecting the *Summary* tab in one API Console will select the *Summary* tab in other API Consoles on the same page that show the same API.
...
The API details button enables users to access details about an API.

* *API*: Choose the API to show.
* *Button Label*: Set the button label. Default is `API Details`.
* *Button Align*: Set button alignment to left, right, or center.
* *API Name*: Choose "Append", "Prepend", or "None" to add the API name to the button label.
* *API Version*: Choose "Append", "Prepend", or "None" to add the API version to the button label.
...
The learn more button enables users to access a custom page with additional information about the API.
