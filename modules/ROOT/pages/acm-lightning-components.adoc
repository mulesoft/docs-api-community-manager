...
When a user requests access to an API instance that has an identity provider (IDP), the user must specify one or more of the OAuth 2.0 grant types *Authorization Code Grant*, *Implicit Grant*, *Refresh Token*, and *Resource Owner Grant*. Some IDPs do not support all grant types. Requesting access and specifying an unsupported grant type shows the error "Error creating application."
