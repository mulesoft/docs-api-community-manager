...
By default the API console sends requests from the browser through Salesforce Experience Cloud to an API, and request size is limited to 2 megabytes or less. To send requests directly from the browser to an API instance, which allows requests with no size limit, open *Setup* > *Custom Metadata Type* > *ACM API Console Configuration Metadata* > *Manage ACM API Console Configuration Metadata* > *ACM API Console Configuration* > *Edit* and select *Browser Requests Enabled*. This setting is applied to all API instances in the community and does not affect the mocking service.
...
By default, the table shows all custom fields listed in the Anypoint organization. To show only some fields, open *Setup* > *Custom Metadata Type* > *ACM General Configuration Metadata* > *Manage Records*, next to *ACM Configuration* click *Edit*, and in the *CustomFields* box enter a custom fields key such as `email,rating,case`.
