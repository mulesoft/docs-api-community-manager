...
The available buttons are *Download Spec as RAML* and *Download Spec as OAS*. If the API specification is uploaded as RAML and downloaded as OAS, or uploaded as OAS and downloaded as RAML, then the downloaded version is automatically generated and may have small inconsistencies with the uploaded version.
...
Layout:: Specifies the layout of the buttons, `Horizontal` or `Vertical`. Default is `Horizontal`.
