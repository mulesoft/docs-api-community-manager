...
When users need a client ID and client secret to access an API, they can select an API instance in the *Credential source* list to populate the client ID and client secret from that instance automatically, or select *Custom credentials* in the list to enter the client ID and client secret manually.
