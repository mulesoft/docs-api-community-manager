...
*Enable advance links*:: Enables search results to link to specific tabs of an API details page identified by tab set IDs, such as the console, documentation, custom fields, and applications tabs.
...
. Select a tab.
. In the URL find a tab set ID such as `tabset-1a2b3c=2` or `tabset-1a2b3c=4d5e6f`.
...
To enable users to select an application to populate the `clientId` and `clientSecret` credentials, add credential annotations to the application specification:
