...
You design the pages of your Anypoint API Community Manager portal by adding and configuring Lightning components in Site Builder.
...
To link directly to a markdown page in this component, set the URL query parameter `selectedPage` to the exact name of the page. For example, if the page is named "Details", use a URL like this: `/communityapi/a003D000001pQpjQAE/basicapi?selectedPage=Details`. If `selectedPage` is defined both in Site Builder for this component and in the URL, then the value in Site Builder is used and the value in the URL is ignored.
...
*Disable Request Access Anypoint Email*:: Disable emails for approved and revoked contracts.
