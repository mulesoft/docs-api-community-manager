...
By default the API console sends requests from the browser through Salesforce Experience Cloud to an API. To send requests directly from the browser to an API instance, open *Setup* > *Custom Metadata Type* > *ACM API Console Configuration Metadata* > *Manage ACM API Console Configuration Metadata* > *ACM API Console Configuration* > *Edit* and select *Browser Requests Enabled*.
