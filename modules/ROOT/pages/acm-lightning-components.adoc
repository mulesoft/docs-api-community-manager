...
By default, the table shows all custom fields listed in the Anypoint organization. To show only some fields, open *Setup* > *Custom Metadata Type* > *ACM General Configuration Metadata* > *Manage Records*, next to *ACM Configuration* click *Edit*, and in the *CustomFieldsFilter* box enter a series of custom field keys such as `email,rating,case`. To list all the custom field keys, open the developer console and run the query `SELECT acm_pkg\__TagKey__c FROM acm_pkg\__AnypointCustomFields__x`.
