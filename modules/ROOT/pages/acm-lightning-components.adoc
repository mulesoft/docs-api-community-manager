...
Search Areas:: Selects which types of content to search. Choices include APIs, Applications, Knowledge Articles, Cases, Managed Content (CMS), Pages, and Forums.
...
If an *API Console* navigation item such as an API endpoint or method has a long name, it wraps around to a second line. By default, the line breaks at the end of a word. To configure the line break behavior:
...
<style>
    api-navigation {
      --api-navigation-list-item-word-break: keep-all !important;
    }
</style>
----
. Optionally, replace `api-navigation` with any selector that contains the component `api-navigation`.
. Optionally, replace `keep-all` with any valid value for the `word-break` CSS property.
+
The default behavior is `break-word`.
...
To link directly to a markdown page in this component, set the URL query paramter `selectedPage` to the exact name of the page. For example, if the page is named "Details", use a URL like this: `/communityapi/a003D000001pQpjQAE/basicapi?selectedPage=Details`. If `selectedPage` is defined in the Community Builder for this component and defined in the URL, then the value in Community Builder is used and the value in the URL is ignored.
...
Show API Version:: Shows the asset version from Exchange.
...
Advanced Filter:: Shows only APIs that meet the SOQL condition. Conditions can use the fields `Name`, `Environment`, and `URL`. For example: `Name LIKE 'v1%'`
...
Download Spec Options:: Specifies the download buttons to show, *Original Format* (default), *All*, *OAS3*, or *RAML*.
