...
. Ensure your Salesforce organizations have the "External Data Change Tracking for OData 4.0 Adapter" feature enabled.
...
.. If you don't see a checkbox field with the label *Eligible for External Change Data Capture*, raise a support case through the MuleSoft support portal, and include the organization IDs of all your Salesforce API Community Manager organizations, including production and all sandboxes.
+
An attempt to upgrade the package without this feature enabled causes the process to fail with errors due to missing objects and fields related to a missing "ChangeEvent" object.
