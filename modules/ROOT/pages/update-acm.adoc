...
After you upgrade using this process, you cannot undo the installation and return to a previous version. Follow the best-practice workflow by creating safe sandbox environments and using them to test each upgrade before installing the new package in your production environment. For more information, see the documentation page on sandboxes.
