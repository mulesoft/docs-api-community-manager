...
. If you are upgrading from a package version before 1.82, follow the <<post-182>>.
...
[[post-182]]
== Postinstallation Steps When Upgrading from a Version Before 1.82

If you are upgrading from a package version before 1.82, use either the automatic or manual configuration methods to update the permissions of the *Member* and *Guest* profiles on the *AnypointAssetCustomFields* and *AnypointCustomFields* objects, and ensure both *Member* and *Guest* profiles have field-level *Read* access to the *AssetVersionId* field in the *AnypointApiInstances* object:
