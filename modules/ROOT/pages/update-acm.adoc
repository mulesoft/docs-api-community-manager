...
For information about update notifications, see the documentation page on important notifications.

After you install an update with this process, it is not possible to undo the installation and return to a previous version. Follow the best-practice workflow by creating safe sandbox environments and using them to test each update before installing the update in your production environment. For more information, see the documentation page on sandboxes.
