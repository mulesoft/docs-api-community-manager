...
. If you are upgrading from a package version before 1.80.1, follow the <<post-1801>> and then continue with these instructions.
. If you are upgrading from a package version before 1.80.5, follow the <<post-1805>>.
...
[[post-1805]]
== Postinstallation Steps When Upgrading from a Version Before 1.80.5

If you are upgrading from a package version before 1.80.5, use either the automatic or manual configuration methods to update the permissions of the *Member* and *Guest* profiles on the *AnypointAssetCustomFields* and *AnypointCustomFields* objects, and ensure both *Member* and *Guest* profiles have field-level *Read* access to the *AssetVersionId* field in the *AnypointApiInstances* object:
