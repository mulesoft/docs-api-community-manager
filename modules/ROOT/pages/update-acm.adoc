...
If you are upgrading from a package version earlier than 2.0, use either the automatic or manual configuration methods to update the permissions of the *Member* and *Guest* profiles must have field-level *Read* access to the *AssetId* field in the *AnypointApiInstances* object.
