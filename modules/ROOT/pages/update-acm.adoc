...
Upgrading from a package version before 1.76 requires mandatory configuration updates. When the new package is installed, user access to assets in the community is suspended. When the configuration is complete, user access to asssets in the community is enabled.
...
. Scroll to the *CommunityAssetVersion Sharing Rules* section near the bottom of the page and click *New*.
. In the *Rule Name* section, update *Label* with `CommunityAssetVersion`.
...
. In the *Select Objects* section, select *CommunityAssetVersion* from *Available Objects* and add it to *Selected Objects*.
. In the *Configure Access* section, select *Set Up* under the *Action* column next to *CommunityAssetVersion*.
...
. For the object *CommunityAssetVersions*, in *Custom Object Permissions* ensure *Read* and *Create* are enabled.
