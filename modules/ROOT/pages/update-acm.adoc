...
To create a new user, you must use API Community Manager Diagnostics or manually set the user profile to the corresponding permission set. 
...
. If the query lists any users, change their status to active with the https://help.salesforce.com/s/articleView?language=en_US&id=000336144&mode=1&type=1[reactivate a user] instructions.
...
* https://www.salesforce.com/en-us/wp-content/uploads/sites/4/documents/legal/Agreements/versioning-back-support-policy.pdf[Product Versioning and Back Support Policy, role=external, window=blank]
