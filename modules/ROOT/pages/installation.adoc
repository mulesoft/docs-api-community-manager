...
omit the first step of signing up for a developer org in salesforce???????????????????????????assume the user will already have a salesforce org
...
then troubleshooting and reference: auth provider details should be under `Further reading`
...
. Navigate to *Field-Level Security* and in the *Custom Field-Level Security* section, change the permissions for the following objects: ???? (page 20)

Click on View and then edit.
AnypointApiVersions, AnypointApplications, AnypointContracts, AnypointApiInstances - which fields need to be updated?????
CommunityApi
...
Select *New*.
and in *Label* enter <code>[Community Name] Public Group<code>.
In *Search*, enter *Users* and then add <code>User: [Community Name] Site Guest User</code> to the *Selected Members* column??????????.
Select *Save*.


Create *Sharing Settings* that will use the Public Group previously created.

. Navigate to *Setup* ->*Security* -> *Sharing Settings*.
. Scroll to and select *CommunityApi Sharing Rules* and select *New*.
...

Then switch to salesforce classic (how????), select the plus button and in the *View* component, select ‘Content’ and then select *Libraries*.
...
If you do not see the content of the ACM Community Manager in the *Lightning Experience App Manager*, you will have to add the ‘ACM Administrator’ tab from the App Manager 
To do this, in *Setup*, search for App Manager in the Quick Find box, then select API Community Manager and edit it. 

Navigate to *Navigation Items*, highlight *ACM Administrator*, and move it to the *Selected Items* column. Select *Save*.

Log out and log back in to the organization. You can then configure the API Community Manager.

In *ACM Administrator* (how do we get here????), select *Manage APIs*. You can select which API will appear in the Community portal using *Add to Community*. 
You can change the description, icon, visibility and the marketing page of the APIs with the edit button in the *APIs in this Community* section.
...
Then navigateto the community builder go to Settings > General > click on the Guest User Profile link. Your Guest Community Profile detail page is displayed.
...
Click on save.
And then you have to change the permissions for the following objects in the *Custom Field-Level Security* section:
...
Select option All on Search drop-down.
...
Change the Login Icon information in *Branding Options*.
...

Define moderation rules for forums
Go to the Community Workspace > Moderation > Rules (top navigation bar) and create a rule. At a minimum, enter information for the *Name* and *Unique Name* fields in the *Details* section MAKE SURE “Activate Rule” CHECKBOX IS ACTIVE and save the changes. Enter information for all fields in the *Rule Conditions* section.
...
Click *Go!*.
Skip step 2. (next step)



If you are not prompted to add an Approval step, type  "Approval Processes" on the quick search box and follow these steps:
